...
export const CoseguroTableInline: React.FC = () => {
  ...
  return (
    <div id="tabla-coseguros-planes" className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden space-y-0 scroll-mt-20">
      ...
      {/* ── Section Header ── */}
      <div className="bg-gradient-to-r from-blue-950 via-indigo-900 to-slate-900 px-6 sm:px-8 py-6 text-white">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-1.5 text-[10px] font-extrabold uppercase tracking-widest px-3 py-1 rounded-full bg-blue-500/20 text-blue-200 border border-blue-400/30 mb-2">
              <Sparkles className="w-3.5 h-3.5 text-blue-400" />
              <span>NÚCLEO DE COBERTURA · DSS CPCE SANTA FE</span>
            </div>
            <h2 className="text-xl sm:text-2xl font-extrabold text-white leading-tight flex items-center gap-2">
              <Table className="w-6 h-6 text-blue-400 shrink-0" />
              Tabla Comparativa de Cobertura y Coseguros
            </h2>
            <p className="text-xs sm:text-sm text-slate-300 mt-1 max-w-3xl leading-relaxed">
              Consulta en tiempo real los aranceles, porcentajes de cobertura y períodos de carencia comparados entre el <strong className="text-white font-bold">Plan General</strong> y el <strong className="text-white font-bold">Plan Básico</strong>.
            </p>
          </div>

          <div className="shrink-0 bg-white/10 backdrop-blur-xs px-4 py-2.5 rounded-2xl border border-white/15 text-right">
            <span className="text-[11px] text-slate-300 font-medium block">Total Prestaciones</span>
            <span className="text-lg font-black text-white">{filteredData.length} de {data.length}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-50 border-b border-slate-200 p-4 sm:p-5 space-y-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-11 pr-24 py-2.5 bg-white border border-slate-300 rounded-2xl text-xs sm:text-sm font-semibold text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-2xs"
          />
          {searchTerm && (
            <button
              onClick={() => setSearchTerm('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 px-2.5 py-1 text-xs font-bold text-slate-500 hover:text-slate-800 bg-slate-100 rounded-lg transition-all"
            >
              Limpiar
            </button>
          )}
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar text-xs">
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl font-bold whitespace-nowrap transition-all cursor-pointer shrink-0 text-[11px] ${
      ...
      <div className="p-4 sm:p-6 space-y-4 bg-slate-50/40">
        ...
        <div className="p-3.5 bg-blue-50/90 border border-blue-200/80 rounded-2xl flex items-start gap-3">
          <ShieldCheck className="w-5 h-5 text-blue-600 shrink-0 mt-0.5" />
          <p className="text-xs text-slate-700 leading-relaxed">
            <strong className="text-blue-900 font-bold">Resumen de liquidación:</strong> Los porcentajes indican la cobertura a cargo del DSS. Los coseguros se incorporan a tu resumen mensual de cuenta corriente sin necesidad de abonar efectivo en mostrador.
          </p>
        </div>

        {/* Mobile Swipe Banner */}
        <div className="md:hidden flex items-center justify-between gap-2 px-3.5 py-2 bg-indigo-50 border border-indigo-200 text-indigo-900 rounded-xl text-[11px] font-bold shadow-2xs">
        ...
        <div className="hidden sm:block overflow-x-auto rounded-2xl border border-slate-200 shadow-xs bg-white relative">
          <table className="w-full text-xs border-collapse min-w-[620px]">
            <thead className="bg-slate-900 text-white font-bold text-left">
              <tr>
                <th className="py-3.5 px-4 text-xs font-extrabold w-5/12">Prestación / Detalle</th>
                ...
                <th className="py-3.5 px-3 text-center text-xs font-extrabold">Período de Carencia</th>
              </tr>
            </thead>
              ...
              {filteredData.length > 0 ? (
                filteredData.map((row, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3 px-3 text-center bg-blue-50/30 border-x border-blue-100">
                      <span className={`inline-block px-3 py-1 rounded-xl font-extrabold text-xs ${
                        row.planGeneral.includes('100%')
                          ? 'bg-blue-600 text-white shadow-2xs'
                          : row.planGeneral.includes('Sin Cobertura')
                          ? 'bg-slate-100 text-slate-400'
                ...
                <tr>
                  <td colSpan={4} className="py-12 text-center text-slate-500">
                    <p className="text-sm font-semibold">No se encontraron prestaciones para &quot;{searchTerm}&quot;</p>
                    <button
                      onClick={() => { setSearchTerm(''); setSelectedCategory('Todas'); }}
                      className="mt-2 text-xs font-bold text-blue-600 hover:underline"
                    >
      ...
      <div className="px-6 py-3.5 bg-slate-100/80 border-t border-slate-200 flex items-center justify-between text-xs text-slate-600 font-semibold">
        <span>Mostrando {filteredData.length} prestaciones registradas en el Departamento de Servicios Sociales</span>
        <button
          onClick={() => {

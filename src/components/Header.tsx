...
export const Header: React.FC<HeaderProps> = ({ activeTab = 'guia', onSelectNav }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 transition-all duration-300">
      ...
      <div
        className="text-slate-100 text-[11px] sm:text-xs py-1.5 border-b border-blue-700/30 shadow-xs"
        style={{
          background: 'linear-gradient(90deg, #1b3d82 0%, #1a3674 45%, #182e60 72%, #4a2212 88%, #6b2e10 100%)'
        }}>
      ...
      {/* Main navbar — frosted gradient glass in Hero, solid white when scrolled past Hero */}
      <div
        className={`transition-all duration-300 ease-in-out ${
          isPastHero
            ? 'bg-white/98 backdrop-blur-xl border-b border-slate-200/80 shadow-md shadow-slate-200/40'
            : 'backdrop-blur-md border-b border-transparent shadow-none'
        }`}
        style={
          !isPastHero
            ? { background: 'linear-gradient(180deg, rgba(255, 255, 255, 0.65) 0%, rgba(255, 255, 255, 0.25) 70%, rgba(255, 255, 255, 0) 100%)' }
            : undefined
        }>
      ...
      <div className={`md:hidden transition-all duration-300 ease-in-out overflow-hidden ${
        ...
        <div className="bg-white/98 backdrop-blur-2xl border-b border-slate-100 shadow-2xl px-3 py-3 space-y-0.5">
          {NAV_ITEMS.map((item) => {
            const Icon = item.icon;
            const isVademecum = item.id === 'vademecum';
            const isCoberturas = item.id === 'coberturas-planes';
